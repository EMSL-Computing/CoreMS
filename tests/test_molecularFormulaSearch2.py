...
def test_run_molecular_formula_search():
    ...
    abundance = [1, 0.4]
    rp, s2n = [[1, 1],[1, 1]]    
    mass_spectrum_obj = ms_from_array_centroid(mz, abundance, rp, s2n, 'single mf search', polarity=1, auto_process=False)
    ...
    mass_spectrum_obj.molecular_search_settings.usedAtoms = {'C': (1,57) , 'H': (4,200), 'N': (0,1)}
...
def test_mspeak_search(mass_spectrum_ftms):
    ...
    mass_spec_obj.molecular_search_settings.usedAtoms = {'C': (1,100) , 'H': (4,200), 'O': (0,10), 'N': (0,1), 'P': (0,1)}
    ...
    assert round(mspeak_obj[0].mz_error, 2) == 1.46
