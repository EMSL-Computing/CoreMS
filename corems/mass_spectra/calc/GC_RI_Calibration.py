...
def get_rt_ri_pairs(gcms_ref_obj, sql_obj=None):
    ...
    lowResSearch = LowResMassSpectralMatch(gcms_ref_obj,sql_obj=sql_obj, calibration=True)
    ...
    for gcms_peak in gcms_ref_obj:
        ...
        if gcms_peak:
            ...
            if not compound_obj.ri in dict_ri_rt.keys():
                ...
                dict_ri_rt[compound_obj.ri] = [(gcms_peak.mass_spectrum.retention_time, compound_obj)]

            else:
                
                dict_ri_rt[compound_obj.ri].append((gcms_peak.mass_spectrum.retention_time, compound_obj))
            
            print(compound_obj.name, gcms_peak.mass_spectrum.retention_time, compound_obj.spectral_similarity_score) #TODO KRH: remove print after encapusulating in a verbose flag
