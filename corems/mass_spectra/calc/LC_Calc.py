...
class PH_Calculations:
    ...
    * find_c13_mass_features(verbose=True).
    ...
    def grid_data(self, data, verbose=True):
        ...
        dat_mz = data_w[['mz', 'intensity']].sort_values(by=['intensity'], ascending=False)
        ...
        mz_dat_np = dat_mz[['mz']].sort_values(by=['mz']).reset_index(drop=True).values.flatten()
        ...
        new_data_w = new_data_w.drop(columns=['mz_diff', 'mz_orig']).groupby(['scan', 'mz', 'scan_time'])['intensity'].sum().reset_index()
        new_data_w = new_data_w.sort_values(by=['scan', 'mz'], ascending = [True, True]).reset_index(drop=True).copy()
    ...
    def find_mass_features(self, ms_level=1, verbose=True, grid=True):
        ...
        if ms_level not in self._ms_unprocessed.keys():
            raise ValueError("No MS level " + str(ms_level) + " data found, did you instantiate with parser specific to MS level?")
        ...
        smooth_radius=[self.parameters.lc_ms.ph_smooth_radius_mz, self.parameters.lc_ms.ph_smooth_radius_scan] #mz, scan_time smoothing radius (in steps)
        ...
        mass_features = peaks.sort_values(by='persistence', ascending=False).reset_index(drop=True)

        # Filter by persistence threshold
        persistence_threshold = self.parameters.lc_ms.ph_persis_min
        mass_features = mass_features.loc[mass_features['persistence'] > persistence_threshold, :].reset_index(drop=True)
    ...
    def cluster_mass_features(self, drop_children=True, verbose=True):
        ...
        if drop_children is True:
            ...
            self.mass_features = {k: v for k, v in self.mass_features.items() if k not in cluster_daughters}
        else:
    ...
    def integrate_mass_features(self, drop_if_fail = False, ms_level = 1):
        ...
        scan_df_sub = self.scan_df[self.scan_df['ms_level'] == int(ms_level)].reset_index(drop=True)
        ...
        for mz in mzs_to_extract:
            ...
            raw_data_sub = raw_data[(raw_data['mz'] >= mz_min) & (raw_data['mz'] <= mz_max)].reset_index(drop=True)
            raw_data_sub = raw_data_sub.groupby(['scan'])['intensity'].sum().reset_index()
        ...
        for idx, mass_feature in mf_df.iterrows():
            ...
            centroid_eics = self.eic_centroid_detector(myEIC.time, myEIC.eic_smoothed, mass_feature.intensity*1.1, apex_indexes=[int(apex_index)])
            l_a_r_scan_idx = [i for i in centroid_eics]
            if len(l_a_r_scan_idx) > 0:
                # Add start and final scan to mass_features and EICData
                left_scan, right_scan = myEIC.scans[l_a_r_scan_idx[0][0]], myEIC.scans[l_a_r_scan_idx[0][2]]
                ...
                area = np.trapz(myEIC.eic_smoothed[l_a_r_scan_idx[0][0]:l_a_r_scan_idx[0][2]+1], myEIC.time[l_a_r_scan_idx[0][0]:l_a_r_scan_idx[0][2]+1])
    ...
    def find_c13_mass_features(self, verbose=True):
        ...
        if verbose:
            ...
            print(str(round(len(mf_df[mf_df['c13_flag'] == 1]) / len(mf_df), ndigits=3)) + " of mass features have or are C13 isotopes")